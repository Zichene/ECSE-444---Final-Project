...
static int wifi_send_to_other_board(void);
static int wifi_connect_to_ap(char* ssid, char* password, uint8_t security);
...
/**
 * This function should send some data to another B-L4S5I board
 */
static int wifi_send_to_other_board(void) {
	// this contains all the important data to be sent
	int data[100];
	// initiates the wifi module
	wifi_start();

	// connecting to existing AP
	if (wifi_connect_to_ap("Dinowifi", "shuangdu1976!", WIFI_ECN_WPA2_PSK) != WIFI_STATUS_OK) {
		printf("Could not connect to AP: check SSID & Password information \r\n");
		return -1;
	}
	...
	// waiting for other board to connect to this server, TRY
	 // making the other board call ""
	  uint8_t RemoteIP[4];
	  uint16_t RemotePort;
...
static int wifi_test(void) {
	...
	if (wifi_connect_to_ap(ssid, password, security) != WIFI_STATUS_OK) {
		printf("Could not connect to AP: check SSID & Password information \r\n");
		return -1;
	}
	 ...
	 // starting server
	  ...
	  while (true) {
		  if (WIFI_STATUS_OK == WIFI_ReceiveData(SOCKET, resp, 1000, &respLen, WIFI_READ_TIMEOUT)) {
			  if(respLen > 0 && strstr((char *)resp, "POST")) {
...
int wifi_connect(void)
{
  ...
  if (user_config.wifi_config_magic != USER_CONF_MAGIC)
  {
   ...
   //scanf("%s", user_config.wifi_config.ssid);
    ...
    strcpy(user_config.wifi_config.ssid,"Dinowifi");
    ...
    if (user_config.wifi_config.security != 0)
    {
      ...
      strcpy(user_config.wifi_config.password, "shuangdu1976!");
    }
