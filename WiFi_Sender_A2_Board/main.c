...
int main(void)
{
 ...
 // HAL_ADCEx_Calibration_Start(&AdcHandle, ADC_SINGLE_ENDED);
  ...
  char* ssid = "iPhone de Qihao56";
  char* password = "ozjwk0kyq9hi9";
  uint8_t security = WIFI_ECN_WPA2_PSK;
  //WIFI_ConfigureAP(ssid, password, security, 1, 8);
    ...
    static uint8_t RemoteIP_Addr[4];
    RemoteIP_Addr[0] = 172;
    RemoteIP_Addr[1] =  20;
    RemoteIP_Addr[2] =  10;
    RemoteIP_Addr[3] =   3;
    if (WIFI_OpenClientConnection(0, WIFI_TCP_PROTOCOL, "test", RemoteIP_Addr, 80, 10) != WIFI_STATUS_OK) {
    	printf("Could not connect to other board \r\n");
    }
    char* http[200];
    char* text = "some random ass data";
    strcpy((char *)http, (char*)"HTTP/1.0 200 OK\r\nContent-Type: text/plain");
    strcat((char *)http, text);
    uint16_t actualSent;
    if (WIFI_SendDataTo(0, http, sizeof(http), &actualSent, WIFI_WRITE_TIMEOUT, RemoteIP_Addr, 80)!= WIFI_STATUS_OK) {
    	printf("Could not send data rip \r\n");
    }
    printf("Sent out %d\r\n", actualSent);
